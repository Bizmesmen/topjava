...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExcess> filteredByCycles(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        for (UserMeal newMeal : meals) {
            int currentCalories = dateCalories.getOrDefault(newMeal.getDateTime().toLocalDate(), 0);
            dateCalories.put(newMeal.getDateTime().toLocalDate(), currentCalories + newMeal.getCalories());
        }
        ...
        for (UserMeal meal : meals) {
            if (isBetweenInclusive(meal.getDateTime().toLocalTime(), startTime, endTime)) {
                completedResult.add(new UserMealWithExcess(
                        meal.getDateTime(),
                        meal.getDescription(),
                        meal.getCalories(),
